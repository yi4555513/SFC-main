...
from main.solver.learning.reinforcement_learning.safe_rl_solver.net import ActorCritic
...
from main.core import Solution, SolutionStepEnvironment
...
class GnnSeq2SeqInstanceEnv(JointPRStepInstanceRLEnv):
    ...
    def __init__(self, p_net: PhysicalNetwork, v_net: VirtualNetwork, controller: Controller, recorder: Recorder,
                 counter: Counter, logger: Logger, config: DictConfig, **kwargs):
        with open_dict(config):
            config.rl.feature_constructor.name = 'p_net_v_node'
        super(GnnSeq2SeqInstanceEnv, self).__init__(p_net, v_net, controller, recorder, counter, logger, config,
                                                    **kwargs)
...
class A3CGcnSeq2SeqSolver(InstanceAgent, A2CSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, **kwargs):
        InstanceAgent.__init__(self, GnnSeq2SeqInstanceEnv)
        A2CSolver.__init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs)
    ...
    def learn_with_instance(self, instance):
        ...
        last_value = self.estimate_value(self.preprocess_obs(next_instance_obs, self.device)) if hasattr(self.policy,
                                                                                                         'evaluate') else None
...
def make_policy(agent, **kwargs):
    ...
    feature_dim = agent.config.simulation.p_net_setting_num_node_resource_attrs + agent.config.simulation.p_net_setting_num_link_resource_attrs + 5
    policy = ActorCritic(
        p_net_num_nodes=action_dim,
        p_net_x_dim=5,
        v_net_x_dim=2,
        embedding_dim=agent.config.nn.embedding_dim).to(agent.device)
    optimizer = torch.optim.Adam([
        {'params': policy.encoder.parameters(), 'lr': agent.config.rl.learning_rate.actor},

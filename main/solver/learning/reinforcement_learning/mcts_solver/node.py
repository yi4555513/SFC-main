...
class State:
    ...
    def random_select_next_state(self):
        ...
        # next_v_node_id = self.v_node_id + 1
        # src_node = self.v_net.get_graph_attribute("src")
        # dst_node = self.v_net.get_graph_attribute("dst")
        # if next_v_node_id == 0:
        #     candidate_p_nodes = [src_node]
        # elif next_v_node_id == len(self.v_net.nodes)-1:
        #     candidate_p_nodes = [dst_node]
        # else:
        #     filter_nodes = self.selected_p_net_nodes + [src_node, dst_node]
        #     candidate_p_nodes = self.controller.find_candidate_nodes(
        #         v_net=self.v_net,
        #         p_net=self.p_net,
        #         v_node_id=self.v_node_id+1,
        #         filter=filter_nodes)
        filter_nodes = self.selected_p_net_nodes
        candidate_p_nodes = self.controller.find_candidate_nodes(
            v_net=self.v_net,
            p_net=self.p_net,
            v_node_id=self.v_node_id + 1,

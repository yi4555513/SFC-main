...
#     return data
# TODO
def get_pyg_data(x, edge_index, edge_attr=None, edge_index_2hop=None, edge_attr_2hop=None, sort_index=False):
    """
    将节点和边信息转换为 PyTorch Geometric Data 格式。

    参数：
        x (ndarray): 节点特征。
        edge_index (ndarray): 1-hop 边索引。
        edge_attr (ndarray, optional): 1-hop 边属性。
        edge_index_2hop (ndarray, optional): 2-hop 边索引。
        edge_attr_2hop (ndarray, optional): 2-hop 边属性。
        sort_index (bool): 是否对边索引排序。

    返回：
        Data: 包含 1-hop 和 2-hop 边信息的 PyG Data 对象。
    """
    x = torch.tensor(x, dtype=torch.float32)
    edge_index = torch.tensor(edge_index, dtype=torch.long)
    edge_attr = torch.tensor(edge_attr, dtype=torch.float32) if edge_attr is not None and edge_attr.size > 0 else None
    ...
    if edge_index_2hop is not None and edge_index_2hop.size > 0:
        data.edge_index_2hop = torch.tensor(edge_index_2hop, dtype=torch.long)
    if edge_attr_2hop is not None and edge_attr_2hop.size > 0:
        data.edge_attr_2hop = torch.tensor(edge_attr_2hop, dtype=torch.float32)

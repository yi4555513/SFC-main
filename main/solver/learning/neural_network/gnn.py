...
from torch_geometric.nn import MessagePassing, GCNConv, GATConv, PNAConv, NNConv, SAGEConv, global_add_pool, \
...
class GraphConvNet(nn.Module):
    ...
    def forward(self, input):
        ...
        # TODO
        x, edge_index, edge_attr, edge_index_2hop, edge_attr_2hop = input['x'], input['edge_index'], input.get(
            'edge_attr', None), input['edge_index_2hop'], input.get('edge_attr_2hop', None)
        y = x
        for layer_id in range(self.num_layers):
            ...
            y = conv(y, edge_index_2hop, edge_attr_2hop)
            if layer_id == self.num_layers - 1:
                x = dout(norm(x))
                y = dout(norm(y))
            else:
                x = F.leaky_relu(dout(norm(x)))
                y = F.leaky_relu(dout(norm(y)))

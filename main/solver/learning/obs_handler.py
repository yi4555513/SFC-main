...
from main.network.topology.topological_metric_calculator import TopologicalMetrics
...
class ObservationHandler:
    ...
    @staticmethod
    def _get_benchmark(benchmarks: Optional[Dict[str, float]], attr_name: str, data: np.ndarray) -> float:
        if benchmarks is not None and attr_name in benchmarks:
            value = float(benchmarks[attr_name] or 0.0)
        else:
            value = float(np.max(np.abs(data)) if np.size(data) else 0.0)
    ...
    def _get_attr_benchmarks(self, attr_types: list, attrs_list: list, attr_data: np.ndarray) -> dict:
        ...
        Args:
            attr_types: A list of specified types of attributes.
            attrs_list: A list of specified attributes.
            attr_data: The data of these attributes.
        ...
        if 'extrema' in attr_types:
            for attr, attr_data in zip(attrs_list, attr_data):
                # Skip resource attributes or handle them differently
                if attr.type == 'resource':
                    continue
                elif attr.type == 'latency':  # Handle latency type separately
                    attr_benchmarks[attr.name] = attr_data.max()  # Assuming you want max for latency
                elif attr.type == 'extrema':
                    attr_benchmarks[attr.originator] = attr_data.max()
                else:
                    attr_benchmarks[attr.name] = attr_data.max()
            ...
            for attr, attr_data in zip(attrs_list, attr_data):
                if attr.type == 'latency':  # Handle latency type separately
                    attr_benchmarks[attr.name] = attr_data.max()  # Assuming you want max for latency
                else:
                    attr_benchmarks[attr.name] = attr_data.max()
        return attr_benchmarks
    ...
    def get_node_attrs_obs(self, network: BaseNetwork, node_attr_types=['resource'], node_attr_benchmarks: Optional[Dict[str, float]] = None):
        ...
        if node_attr_benchmarks is not None:
            for i, n_attr in enumerate(n_attrs):
                ...
                node_data[i] = node_data[i] / self._get_benchmark(node_attr_benchmarks, attr_name, node_data[i])
        return node_data.T

    def get_link_attrs_obs(self, network: BaseNetwork, link_attr_types=['resource','latency'], link_attr_benchmarks: Optional[Dict[str, float]] = None):
        ...
        if link_attr_benchmarks is not None:
            for i, l_attr in enumerate(l_attrs):
                ...
                assert isinstance(attr_name, str), f"attr_name should be str, but got {type(attr_name)}"
                link_data[i] = link_data[i] / self._get_benchmark(link_attr_benchmarks, attr_name, link_data[i])
        return link_data.T

    def get_link_sum_attrs_obs(self, network: BaseNetwork, link_attr_types: list = ['resource','latency'], link_sum_attr_benchmarks: Optional[Dict[str, float]] = None):
        return self.get_link_aggr_attrs_obs(network, link_attr_types, aggr='sum', link_sum_attr_benchmarks=link_sum_attr_benchmarks)

    def get_link_aggr_attrs_obs(self, network: BaseNetwork, link_attr_types: list = ['resource','latency'], aggr='sum', link_sum_attr_benchmarks: Optional[Dict[str, float]] = None, link_attr_benchmarks: Optional[Dict[str, float]] = None):
        l_attrs = network.get_link_attrs(link_attr_types)
        ...
        if aggr=='sum' and link_sum_attr_benchmarks is not None:
            for i, l_attr in enumerate(l_attrs):
                attr_name = l_attr.originator if l_attr.type == 'extrema' else l_attr.name
                link_aggr_attrs_data[i] = link_aggr_attrs_data[i] / self._get_benchmark(
                    link_sum_attr_benchmarks, attr_name, link_aggr_attrs_data[i]
                )
        elif aggr in ['mean', 'max', 'min'] and link_attr_benchmarks is not None:
            for i, l_attr in enumerate(l_attrs):
                attr_name = l_attr.originator if l_attr.type == 'extrema' else l_attr.name
                link_aggr_attrs_data[i] = link_aggr_attrs_data[i] / self._get_benchmark(
                    link_attr_benchmarks, attr_name, link_aggr_attrs_data[i]
                )

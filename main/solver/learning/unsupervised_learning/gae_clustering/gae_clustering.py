...
from main.solver.learning.utils import get_available_device, load_pyg_data_from_network
...
class GaeClusteringSolver(Solver):
    ...
    """
    def __init__(self, controller, recorder, counter, logger, config, num_features=1, num_clusters=4, **kwargs):
    ...
    def from_config(cls, config,logger):
        ...
        num_features = config.pop('num_v_net_node_attrs', 2)
    ...
    def solve(self, instance) -> Solution:
        ...
        for cluster_id in range(self.num_clusters):
            ...
            while remain_num_attempts != 0:
                ...
                # TODO
                first_vnf = 0
                last_vnf = len(v_net.nodes)-1
                solution = Solution.from_v_net(v_net)
                src = v_net.v_net.get_graph_attribute("src")
                dst = v_net.v_net.get_graph_attribute("dst")
                # 1. 固定部署最后一个 VNF（假设要放在物理节点 12）
                success, _ = self.controller.place_and_route(v_net, p_net, last_vnf, dst, solution)
                if not success:
                    return Solution.from_v_net(v_net)
                # 2. 中间节点部署顺序
                sorted_v_nodes = list(v_net.nodes)
                sorted_v_nodes.remove(last_vnf)  # 只去掉最后一个，保留第一个让算法处理
                # 3. 起点设为你希望第一个 VNF 映射到的物理节点，比如节点 3
                starting_node = src

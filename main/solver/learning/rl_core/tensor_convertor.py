...
from main.solver.learning.utils import get_pyg_data
...
class TensorConvertor:
    ...
    #         # TODO
    #         obs_p_net_edge_index_2hop = obs.get('p_net_edge_index_2hop', None)
    #         obs_p_net_edge_attr_2hop = obs.get('p_net_edge_attr_2hop', None)
    #         tensor_obs_p_net = get_pyg_data(obs['p_net_x'], obs['p_net_edge_index'], obs_p_net_edge_attr,obs_p_net_edge_index_2hop,obs_p_net_edge_attr_2hop,)
    ...
    def p_net_obs_as_tensor(obs, device):
        if isinstance(obs, dict):
            assert 'p_net_x' in obs, "观测字典必须包含 'p_net_x' 键。"
            assert 'p_net_edge_index' in obs, "观测字典必须包含 'p_net_edge_index' 键。"
            obs_p_net_edge_attr = obs.get('p_net_edge_attr', None)
            obs_p_net_edge_index_2hop = obs.get('p_net_edge_index_2hop', None)
            obs_p_net_edge_attr_2hop = obs.get('p_net_edge_attr_2hop', None)
            p_net_data = get_pyg_data(
                obs['p_net_x'],
                obs['p_net_edge_index'],
                obs_p_net_edge_attr,
                obs_p_net_edge_index_2hop,
                obs_p_net_edge_attr_2hop
            )
            tensor_obs_p_net = Batch.from_data_list([p_net_data]).to(device)
            return {'p_net': tensor_obs_p_net}
        elif isinstance(obs, list):
            p_net_data_list = []
            for observation in obs:
                assert 'p_net_x' in observation, "观测字典必须包含 'p_net_x' 键。"
                assert 'p_net_edge_index' in observation, "观测字典必须包含 'p_net_edge_index' 键。"
                obs_p_net_edge_attr = observation.get('p_net_edge_attr', None)
                obs_p_net_edge_index_2hop = observation.get('p_net_edge_index_2hop', None)
                obs_p_net_edge_attr_2hop = observation.get('p_net_edge_attr_2hop', None)
                p_net_data = get_pyg_data(
                    observation['p_net_x'],
            ...
            raise Exception(f"无法识别的观测类型 {type(obs)}")
    @staticmethod
    ...
    def general_obs_as_tensor(obs, device):
        # TODO
        general_obs_item_keys = set(['curr_v_node_id', 'action_mask', 'v_net_size','src','dst'])
        if isinstance(obs, dict):

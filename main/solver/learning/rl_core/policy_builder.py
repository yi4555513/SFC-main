...
from main.solver.learning.rl_policy.gnn_mlp_policy import DeepEdgeFeatureGATActorCritic
...
def get_service_qos_extra_dim(config: Any) -> int:
    """Extra virtual-node features used by Chapter 4 multi-service QoS.

    The feature constructor appends four phi weights plus one CSS key-VNF flag
    when service_qos is enabled.  Keeping this calculation here prevents the
    policy input dimension from staying at the Chapter 3 value.
    """
    try:
        return 5 if bool(config.v_sim_setting.service_qos.enabled) else 0
    except Exception:
        return 0
...
def get_v_node_x_dim(config: Any) -> int:
    ...
    v_node_x_dim += get_service_qos_extra_dim(config)
    return v_node_x_dim

...
from main.solver.heuristic.node_rank import *
...
from main.utils import test_running_time
...
class RLSolver(Solver):
    ...
    def save_model(self, checkpoint_fname):
        ...
        # Save policy/optimizer together with training counters so a later run can
        # continue both optimization and the PPO reward x-axis instead of starting
        # the log again from update_time=0.
        torch.save({
            'policy': self.policy.state_dict(),
            'optimizer': self.optimizer.state_dict(),
            'update_time': int(getattr(self, 'update_time', 0)),
            'time_step': int(getattr(self, 'time_step', 0)),
            # 'lr_scheduler_state_dict': self.lr_scheduler.state_dict()
        }, checkpoint_fname)
        self.logger.critical(f'Save model to {checkpoint_fname}\n')

    def _training_resume_enabled(self, name, default=False):
        training_cfg = getattr(self.config, 'training', {})
        if hasattr(training_cfg, 'get'):
            return bool(training_cfg.get(name, default))
        return bool(getattr(training_cfg, name, default))

    def _max_logged_update_time(self, training_info_path):
        if not training_info_path or not os.path.exists(training_info_path):
            return None
        try:
            with open(training_info_path, 'r', newline='') as f:
                reader = csv.DictReader(f)
                values = []
                for row in reader:
                    try:
                        values.append(int(float(row.get('update_time', ''))))
                    except Exception:
                        pass
    ...
    def _copy_pretrained_training_log(self, checkpoint_path):
        # checkpoint: <run_dir>/models/model.pkl; log: <run_dir>/logs/training_info.csv
        prev_run_dir = os.path.dirname(os.path.dirname(os.path.abspath(checkpoint_path)))
        prev_log_path = os.path.join(prev_run_dir, self.config.logger.log_dir_name, 'training_info.csv')
        curr_log_path = os.path.join(self.log_dir, 'training_info.csv')
        if not os.path.exists(prev_log_path):
            return None
        if os.path.abspath(prev_log_path) == os.path.abspath(curr_log_path):
            return prev_log_path
        if self._training_resume_enabled('append_pretrained_training_log', True):
            os.makedirs(self.log_dir, exist_ok=True)
            if not os.path.exists(curr_log_path):
                shutil.copyfile(prev_log_path, curr_log_path)
                self.logger.critical(f'Copied previous training log to continue reward curve: {curr_log_path}')
        return prev_log_path
    ...
    def load_model(self, checkpoint_path):
        ...
        try:
            ...
            if not isinstance(checkpoint, dict) or 'policy' not in checkpoint:
                ...
                if self._training_resume_enabled('resume_training_state', True):
                    prev_log_path = self._copy_pretrained_training_log(checkpoint_path)
                    logged_update = self._max_logged_update_time(prev_log_path)
                    saved_update = checkpoint.get('update_time', None)
                    if saved_update is None and logged_update is not None:
                        saved_update = logged_update + 1
                    if saved_update is not None:
                        self.update_time = int(saved_update)
                    if 'time_step' in checkpoint:
                        self.time_step = int(checkpoint['time_step'])
            self.logger.critical(
                f'Parameter Initialization: Loaded pretrained model from {checkpoint_path}; '
                f'continue update_time={getattr(self, "update_time", 0)}')
    ...
    def update_grad(self, loss):
        ...
        if self.config.training.distributed_training:
            with self.lock:
                ...
                loss.backward()
                grad_clipped = torch.nn.utils.clip_grad_norm_(self.policy.parameters(),
                                                              self.config.rl.max_grad_norm) if self.config.rl.clip_grad else None
...
class PGSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(PGSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                       **kwargs)
...
class A2CSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(A2CSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                        **kwargs)
        self.repeat_times = 1
...
class PPOSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(PPOSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                        **kwargs)
        self.repeat_times = kwargs.get('repeat_times', int(getattr(config.rl, 'repeat_times', 10)))
    ...
    def update(self, ):
        ...
        for i in range(sample_times):
            ...
            values, action_logprobs, dist_entropy, other = self.evaluate_actions(observations, actions,
                                                                                 return_others=True)
...
class A3CSolver(PPOSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(A3CSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                        **kwargs)
...
class ARPPOSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(ARPPOSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                          **kwargs)
        self.repeat_times = kwargs.get('repeat_times', int(getattr(config.rl, 'repeat_times', 10)))
        self.gae_lambda = kwargs.get('gae_lambda', 0.98)
        self.eps_clip = kwargs.get('eps_clip', 0.2)
...
class DDPGSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(DDPGSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                         **kwargs)
        self.repeat_times = kwargs.get('repeat_times', int(getattr(config.rl, 'repeat_times', 10)))
        self.gae_lambda = kwargs.get('gae_lambda', 0.98)
        self.eps_clip = kwargs.get('eps_clip', 0.2)
...
class DQNSolver(RLSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(DQNSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                        **kwargs)
...
class DoubleDQNSolver(DQNSolver):
    ...
    def __init__(self, controller, recorder, counter, logger, config, make_policy, obs_as_tensor, **kwargs):
        super(DoubleDQNSolver, self).__init__(controller, recorder, counter, logger, config, make_policy, obs_as_tensor,
                                              **kwargs)
        self.target_policy = copy.deepcopy(self.policy)
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            max_next_q_values = self.target_policy.act(next_observations).gather(1, max_next_actions.unsqueeze(
                -1)).squeeze(-1)
            target_q_values = rewards + self.config.rl.gamma * max_next_q_values * (1 - dones)
        # Compute loss
        loss = F.mse_loss(q_values, target_q_values)

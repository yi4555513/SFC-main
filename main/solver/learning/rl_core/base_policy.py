...
from main.solver.learning.neural_network import *
...
class GcnActorCritic(nn.Module):
    ...
    def __init__(self, p_net_num_nodes: int, p_net_x_dim: int, v_node_feature_dim: int, embedding_dim: int = 128,
                 dropout_prob: float = 0., batch_norm: bool = False, **kwargs):
        super().__init__()
...
class GcnActor(nn.Module):
    ...
    def __init__(self, p_net_num_nodes: int, p_net_x_dim: int, v_node_feature_dim: int, embedding_dim: int = 128,
                 dropout_prob: float = 0., batch_norm: bool = False, **kwargs):
        super().__init__()
        self.gnn = GCNConvNet(p_net_x_dim, embedding_dim, embedding_dim=embedding_dim, dropout_prob=dropout_prob,
                              batch_norm=batch_norm, return_batch=True)
        self.mlp = MLPNet(v_node_feature_dim, embedding_dim, num_layers=2, embedding_dims=None,
                          dropout_prob=dropout_prob, batch_norm=batch_norm)
    ...
    def forward(self, obs: dict) -> torch.Tensor:
        ...
        if p_node_embeddings.shape[0] != v_node_embedding.shape[0]:
            raise ValueError(
                f"Batch size mismatch: p_node_embeddings {p_node_embeddings.shape}, v_node_embedding {v_node_embedding.shape}")

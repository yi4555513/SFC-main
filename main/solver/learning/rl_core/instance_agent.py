...
class InstanceAgent(object):
    ...
    def solve(self, instance):
        ...
        instance_env = self.InstanceEnv(p_net, v_net, self.controller, self.recorder, self.counter, self.logger,
                                        self.config)
    ...
    def get_baseline_solution_info(self, instance, if_use_baseline_solver=True):
        ...
        if hasattr(self.baseline_solver, 'unsafe_solve') and self.if_allow_baseline_unsafe_solve:
            ...
            baseline_solution_info = self.counter.count_solution(instance['v_net'], baseline_solution)
            print(
                f"Baseline - Result {baseline_solution_info['result']}, Violation {baseline_solution_info['v_net_total_hard_constraint_violation']}, Is Feasible {baseline_solution.is_feasible()}")
    ...
    def learn_with_instance_parallelly(self, instance):
        ...
        vectorized_env = gym.vector.SyncVectorEnv([
                                                      lambda: self.InstanceEnv(p_net, v_net, self.controller,
                                                                               self.recorder, self.counter, self.logger,
                                                                               self.config)
                                                  ] * 2)
    ...
    def merge_instance_experience(self, instance, solution, instance_buffer, last_value):
        # Record SFC-level reward statistics before the main environment writes
        # one row for this SFC into records/*.csv.  These fields make the reward
        # curve easier to interpret than training_info.csv, whose reward is the
        # mean reward of a PPO update buffer.
        rewards = [float(r) for r in getattr(instance_buffer, 'rewards', [])]
        solution['v_net_episode_reward'] = float(sum(rewards)) if rewards else 0.0
        discounted_return = 0.0
        gamma = float(self.config.rl.gamma)
        for reward in reversed(rewards):
            discounted_return = reward + gamma * discounted_return
        solution['v_net_discounted_return'] = float(discounted_return)
        solution['v_net_num_rl_steps'] = int(len(rewards))
        ...
        if self.config.rl.if_use_negative_sample:
            ...
            if baseline_solution_info['result'] or solution['result']:
                instance_buffer.compute_returns_and_advantages(last_value, gamma=self.config.rl.gamma,
    ...
    def learn_singly(self, env, num_epochs=1, **kwargs):
        ...
        for epoch_id in range(num_epochs):
            ...
            epoch_logprobs = []
            revenue2cost_list = []
            # TODO 初始化总时延和成功的 SFC 数量
            total_latency = 0
            total_successful_sfc = 0
            while True:
                ### --- instance-level --- ###
                ...
                if done:
                    break
            # TODO 计算平均时延
            avg_latency = total_latency / total_successful_sfc if total_successful_sfc > 0 else 0
            epoch_logprobs_tensor = np.concatenate(epoch_logprobs, axis=0)
            end_time = time.time()
            time_cost = end_time - start_time
            # TODO 打印日志，添加平均时延
            self.logger.info(
                f'\nepoch {epoch_id:4d}, success_count {success_count:5d}, r2c {info["long_term_r2c_ratio"]:1.4f}, '
                f'mean logprob {epoch_logprobs_tensor.mean():2.4f}, avg_latency {avg_latency:2.4f} ms, '
            ...
            self.logger.log(
                {
                    ...
                    'train_epochs/avg_latency': avg_latency,  # 将平均时延记录到日志

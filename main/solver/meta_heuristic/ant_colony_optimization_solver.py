...
class Ant(Individual):
    ...
    def __init__(self, id, v_net, p_net, objective_cfg=None):
        super(Ant, self).__init__(id, v_net, p_net)
        self.objective_cfg = objective_cfg if objective_cfg is not None else {}

    def _objective_name(self):
        return str(self.objective_cfg.get('name', 'resource')).lower()

    def _objective_weight(self, name, default):
        try:
            return float(self.objective_cfg.get(name, default))
        except Exception:
            return float(default)

    def calc_fitness(self, solution):
        if not solution['result']:
            return self.infeasible_fitness if hasattr(self, 'infeasible_fitness') else float('inf')

        resource_part = float(solution.get('v_net_cost', 0.0) or 0.0) / max(
            float(solution.get('v_net_revenue', 0.0) or 0.0), 1e-9
        )
        if self._objective_name() not in {'resource_latency', 'latency_resource', 'cost_latency'}:
            return resource_part

        max_latency = self.v_net.graph.get('max_latency', self.objective_cfg.get('latency_norm', 300.0))
        max_latency = max(float(max_latency or 300.0), 1e-9)
        latency_part = float(solution.get('total_latency', 0.0) or 0.0) / max_latency
        return (
            self._objective_weight('resource_weight', 1.0) * resource_part
            + self._objective_weight('latency_weight', 1.0) * latency_part
        )
...
class AntColonyOptimizationSolver(BaseMetaHeuristicSolver):
    ...
    def meta_run(self, v_net: VirtualNetwork, p_net: PhysicalNetwork):
        if self._fix_endpoint_vnfs():
            self._add_endpoint_candidates(v_net)
        ...
        objective_cfg = self._objective_config()

        for iter_id in range(self.max_iteration):
            self.ants = [Ant(i, v_net, p_net, objective_cfg) for i in range(self.num_ants)]
            ant_runners = []
    ...
    def calc_heuristic_info(self, p_net):
        ...
        bw_sum_resource = np.array(
            p_net.get_aggregation_attrs_data(p_net.get_link_attrs('resource'), aggr='sum', normalized=False)).sum(
            0).tolist()
        ...
        for v_node_id in heuristic_info:
            for p_node_id in heuristic_info[v_node_id]:
                heuristic_info[v_node_id][p_node_id] = (heuristic_info[v_node_id][p_node_id] - min_resource) / (
    ...
    def update_pheromone_trail(self):
        ...
        if self.enhence_pheromone in ['all', 'both']:
            ...
            if self.best_individual.best_solution['result']:
                for v_node_id in self.best_individual.v_net.ranked_nodes:
                    ...
                    temp_pheromone_trail[v_node_id][
                        p_node_id] += self.coeff_pheromone_enhancement / self.best_individual.best_fitness
        else:
        ...
        for v_node_id in self.candidates_dict:
            for p_node_id in self.candidates_dict[v_node_id]:
                self.pheromone_trail[v_node_id][p_node_id] = self.pheromone_trail[v_node_id][
                                                                 p_node_id] * self.coeff_pheromone_evaporation \
    ...
    def evolve(self, ant):
        src_v_node = 0
        dst_v_node = len(ant.v_net.nodes) - 1
        if self._fix_endpoint_vnfs():
            ant.v_net.ranked_nodes = [src_v_node, dst_v_node] + [v for v in ant.v_net.ranked_nodes if
                                                                 v not in [src_v_node, dst_v_node]]
        for v_node_id in ant.v_net.ranked_nodes:
            # TODO 淇敼椤哄簭
            if self._fix_endpoint_vnfs() and v_node_id == src_v_node:
                fixed_p_node_for_src = ant.v_net.get_graph_attribute("src")
                result, _ = self.controller.place_and_route(
                    ant.v_net, ant.p_net, src_v_node, fixed_p_node_for_src, ant.solution,
                    shortest_method='bfs_shortest', k=1
                )
                if not result:
                    return
                continue
            if self._fix_endpoint_vnfs() and v_node_id == dst_v_node:
                fixed_p_node_for_dst = ant.v_net.get_graph_attribute("dst")
                result, _ = self.controller.place_and_route(
                    ant.v_net, ant.p_net, dst_v_node, fixed_p_node_for_dst, ant.solution,
                    shortest_method='bfs_shortest', k=1
                )
                if not result:
                    return
            ...
            if p_node_id != -1:
                result, info = self.controller.place_and_route(ant.v_net, ant.p_net, v_node_id, p_node_id, ant.solution,
                                                               shortest_method='bfs_shortest', k=1)
            else:
                break
            if not result:
        ...
        if len(ant.v_net.ranked_nodes) == len(ant.solution['node_slots']) and result:
            ...
            total_latency = self.compute_total_latency(ant.v_net, ant.p_net, ant.solution)
            ant.solution['total_latency'] = total_latency
            max_latency = ant.v_net.graph['max_latency']
            if max_latency is not None and total_latency > max_latency:
                ant.solution.update({'route_result': False, 'result': False, 'description': 'exceed max_latency'})
    ...
    def _add_endpoint_candidates(self, v_net):
        ...
        for v_node_id, p_node_id in endpoints.items():
            if p_node_id not in self.candidates_dict[v_node_id]:
                self.candidates_dict[v_node_id].append(p_node_id)

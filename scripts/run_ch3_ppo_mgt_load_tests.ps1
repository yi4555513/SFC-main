# 第三章 PPO-MGT/ppo_gat_seq2seq+ 不同到达率/负载自动测试脚本
# 用法：powershell -ExecutionPolicy Bypass -File "D:\any download\virne-main\virne-main\scripts\run_ch3_ppo_mgt_load_tests.ps1"
...
$logDir = Join-Path $repo "overnight_logs\ch3_ppo_mgt_load_$timestamp"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null

Set-Location -LiteralPath $repo

Write-Host "============================================================"
Write-Host "第三章 PPO-MGT/ppo_gat_seq2seq+ 负载测试开始"
...
foreach ($rate in $rates) {
    $start = Get-Date
    $logFile = Join-Path $logDir "ppo_mgt_load_lambda_$rate.log"
    Write-Host ""

...
foreach ($alg in $algorithms) {
    foreach ($job in $jobs) {
        Write-Host ""
        Write-Host "============================================================"
        Write-Host "Ch4 dominant ratio: algorithm=$($alg.Label), label=$($job.Label), ratio=$($job.Ratio), service_ratios=$($job.ServiceRatios)"
        Write-Host "============================================================"
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Ch4 dominant ratio failed: algorithm=$($alg.Label), label=$($job.Label), ratio=$($job.Ratio), exit_code=$LASTEXITCODE"
        }
    }
}

Write-Host ""
Write-Host "Finished: Ch4 dominant-ratio tests for PPO-Baseline and ACO-META."
